...
class PreinscriptionService
{
    ...
    public function fetchPreinscriptionDataByInstitute(string $instituteId): array
    {
        ...
        return $this->externalConnection->fetchAllAssociative($sql, ['instituteId' => $instituteId]);
    }
    ...
    public function copyAndAddField(string $filePath): string
    {
        ...
        if (!isset($existingRecords)) {
            // Agregar los encabezados solo si el archivo copiado es nuevo
        ...
        foreach ($csv->getRecords() as $record) {
            $dni = $record['dni_pasaporte']; // Asume que tienes un campo 'dni' en los registros
