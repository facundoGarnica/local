    ...
    public function index(InstitutoRepository $institutoRepository, PersonaRepository $personaRepository, AlumnoRepository $alumnoRepository, Request $request): Response
    {
        ...
        if ($numeroCue) {
            ...
            if ($data && !empty($data)) {
                ...
                $filename = null; // O algún valor predeterminado
            }
        }
        ...
        if ($filename && file_exists($filePath) && $this->isValidCsv($filePath)) {
            $data = $this->preinscriptionService->readCsv($filePath);
        } else {
            $data = null; // No hay datos para procesar
        }
        ...
        if ($filePath2 && file_exists($filePath2) && $this->isValidCsv($filePath2)) {
            $data2 = $this->preinscriptionService->readCsvCopy($filePath2);
        } else {
            $data2 = null; // No hay copia del archivo para procesar
        }
        ...
        if (!empty($personas)) {
            ...
            // Aquí puedes manejar el caso cuando no hay personas
            // Por ejemplo, asignar un valor predeterminado o dejar el arreglo vacío
            $dniList = [];
        }
        ...
        if (!empty($alumnos)) {
            ...
            // Manejar el caso cuando no hay alumnos
            $dniListAlu = [];
        }
